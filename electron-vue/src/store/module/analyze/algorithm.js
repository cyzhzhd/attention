...
function analysis(detection, landmarks, angle, timestamp) {
  ...
  detectVar.absenceTemp = (
    (1 * detectVar.wDetect) /
    (detectVar.wDetect + detectVar.wUndetect)
  ).toFixed(3);
  ...
  // }
  if (
    tempTime.getTime() - detectVar.timeSave.getTime() >
    teacherData.period[0] * 1000
  ) {
    detectVar.timeChange = true;
...
function analyzeFaceExpression(landmarks, timestamp) {
  ...
  if (avgEAR > eyeVar.maxEAR) eyeVar.weight = 0.1;
    ...
    eyeVar.weight =
      1 + Math.pow(Math.abs(avgEAR), 2) - 20 * Math.pow(angleVar.pitch, 2);
  if (angleVar.pitch < 0 || angleVar.pitch > 0.18) eyeVar.weight = 0.1;
  eyeVar.closedRatio = (
    (eyeVar.weight * Math.abs(studentData.eyeSize - avgEAR)) /
  ...
  // }
  if (
    tempTime2.getTime() - eyeVar.timeSave.getTime() >
    teacherData.period[1] * 1000
  ) {
    eyeVar.timeChange = true;
...
function analyzeAngle(angle, timestamp) {
  ...
  angleVar.pitch =
    angleVar.arrPitch.reduce((a, b) => a + b) / angleVar.arrPitch.length;
  angleVar.yaw =
    angleVar.arrYaw.reduce((a, b) => a + b) / angleVar.arrYaw.length;
  angleVar.roll =
    angleVar.arrRoll.reduce((a, b) => a + b) / angleVar.arrRoll.length;

  angleVar.weight = 0.8;
  angleVar.turnRatio =
    (angleVar.weight * (Math.abs(angleVar.yaw) - 0) * (1 - 0)) / (0.3 - 0) + 0;
  ...
  // }
  if (
    tempTime3.getTime() - angleVar.timeSave.getTime() >
    teacherData.period[2] * 1000
  ) {
    angleVar.timeChange = true;
...
function pointCal(timestamp) {
  ...
  else if (pointVar.eyeRatio < 0.27)
    pointVar.eyeWeight = 6.67 * (pointVar.eyeRatio - 0.27) + 0.8;
  else if (pointVar.eyeRatio < 0.34) pointVar.eyeWeight = 0.8;
  else if (pointVar.eyeRatio < 0.4)
    pointVar.eyeWeight = 3.34 * (pointVar.eyeRatio - 0.4) + 1;
  ...
  pointVar.arrPoint.push(
    ((pointVar.eyeWeight * 2.5 +
      pointVar.deltaWeight * 1 +
      pointVar.mouseWeight * 6.5) *
      100) /
      10,
  );
  pointVar.tempPoint =
    pointVar.arrPoint.reduce((a, b) => a + b) / pointVar.arrPoint.length;
  ...
  // }
  if (
    tempTime4.getTime() - pointVar.timeSave2.getTime() >
    teacherData.period[3] * 1000
  ) {
    pointVar.pointTimeChange = true;
