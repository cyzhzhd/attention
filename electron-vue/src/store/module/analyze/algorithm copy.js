...
function getPoint(landmarks, angle) {
    ...
    if (pitch < point.deadP[0]) minusP = point.weightP * (Math.abs(pitch) - point.deadP[0]);
    else if (pitch > point.deadP[1]) minusP = point.weightP * (pitch - point.deadP[1]);
    else minusP = 0;
    ...
    point.center = [point.centerArr0.reduce((a, b) => a + b) / point.centerArr0.length, point.centerArr1.reduce((a, b) => a + b) / point.centerArr1.length];
    const xEdge = [point.centerTemp[0] - point.deadXY[0], point.centerTemp[0] + point.deadXY[0]];
    const yEdge = [point.centerTemp[1] - point.deadXY[1], point.centerTemp[1] + point.deadXY[1]];
    if (nowCenter[0] < xEdge[0] || nowCenter[0] > xEdge[1] || nowCenter[1] < yEdge[0] || nowCenter[1] > yEdge[1]) minusC = 0.3;
    else minusC = 0;

    const mW = calcDist(landmarks[48], landmarks[54]);
    const mH = (calcDist(landmarks[50], landmarks[58]) + calcDist(landmarks[51], landmarks[57]) + calcDist(landmarks[52], landmarks[56])) / 3;
    const MAR = mH / mW;
